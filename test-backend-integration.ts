...
// Test für die korrigierten API-Header und Backend-Format
...
try {
  ...
  const statusTests = [
    { status: 'ONLINE', metadata: { service: 'all-operational' } },
    { status: 'HIGH_LATENCY', metadata: { latencyMs: 2500 } },
    { status: 'ERROR', metadata: { error: 'test-error' } },
  ];

  for (const test of statusTests) {
    try {
      await sentinel.sendStatus(test.status as any, test.metadata);
      console.log(`✅ Status "${test.status}" gesendet`);
    } catch (error) {
      console.log(`❌ Status "${test.status}" fehlgeschlagen:`, error instanceof Error ? error.message : error);
    }
  }
