...
async function fixedHeartbeatExample() {
	...
	try {
		console.log(
			"🚀 Initialisiere SentinelGuard mit Standard-Heartbeat-Daten...",
		);
		...
		console.log(
			"📋 Standard-Heartbeat-Daten gesetzt:",
			sentinel.getDefaultHeartbeatData(),
		);
		...
		console.log(
			"⏳ Warte 15 Sekunden um automatische Heartbeats zu beobachten...",
		);
		console.log(
			"   (Alle 5 Sekunden sollte ein Heartbeat mit Standard-Daten gesendet werden)",
		);
		...
		const watchInterval = setInterval(() => {
			counter++;
			console.log(
				`   📊 Sekunde ${counter * 3} - Heartbeat aktiv: ${sentinel.isHeartbeatActive()}, Fehler: ${sentinel.getHeartbeatErrorCount()}`,
			);
		...
		if (specificHeartbeat.success) {
			console.log(
				"✅ Spezifischer Heartbeat gesendet:",
				specificHeartbeat.data,
			);
		}
