...
async function errorHandlingExample() {
  ...
  const invalidConfigs = [
    ...
    },
    {
      ...
      config: { baseUrl: 'https://api.example.com' } as any,
    },
    {
  ...
  for (const testConfig of invalidConfigs) {
    ...
    try {
      SentinelGuard.validateConfig(testConfig.config);
      console.log('❌ Validierung sollte fehlschlagen');
    } catch (error) {
      console.log('✅ Validierungsfehler erwartet:', error instanceof Error ? error.message : error);
    }
  ...
  try {
    ...
    console.log('✅ Retry-Verhalten getestet:', error instanceof Error ? error.message : error);
  }
