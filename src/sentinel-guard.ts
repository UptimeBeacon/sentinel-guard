...
export class SentinelGuard {
   ...
   */
  async createMonitor(request: CreateMonitorRequest): Promise<ApiResponse<Monitor>> {
   ...
   */
  async updateMonitor(id: string, request: UpdateMonitorRequest): Promise<ApiResponse<Monitor>> {
   ...
   */
  async getMonitorHistory(id: string, limit?: number): Promise<ApiResponse<unknown[]>> {
  ...
  async sendHeartbeat(data?: Partial<HeartbeatData>): Promise<ApiResponse<HeartbeatResponse>> {
    if (!this.heartbeatManager) {
      throw new Error('Heartbeat manager not initialized. Call initializeHeartbeat() first.');
    }
   ...
   */
  async sendStatus(status: 'ONLINE' | 'OFFLINE' | 'HIGH_LATENCY' | 'ERROR', metadata?: Record<string, unknown>): Promise<ApiResponse<HeartbeatResponse>> {
    if (!this.heartbeatManager) {
      throw new Error('Heartbeat manager not initialized. Call initializeHeartbeat() first.');
    }
  ...
  static validateConfig(config: SentinelGuardConfig): boolean {
    ...
    if (config.retryConfig) {
      ...
      if (typeof backoffMultiplier !== 'number' || backoffMultiplier <= 0) {
        throw new Error('retryConfig.backoffMultiplier must be a positive number');
      }
    }
